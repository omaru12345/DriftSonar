...
- [ ] 投稿処理の完了を待ってから `isPosting` を戻す（onSubmit を async/completion 化）
- [ ] 投稿成功までシートを閉じず、失敗時はエラー表示してシートを維持
- [ ] 投稿中は二重送信を防止（ボタン無効化）

**影響ファイル**: `DriftSonarApp/DriftSonarApp/Views/ComposeView.swift`、`DriftSonarApp/DriftSonarApp/Views/TimelineView.swift`、`ViewModels/TimelineViewModel.swift`
...
- [ ] `EncounterView.swift:92`・`TimelineView.swift:76` の `?? Data()` を排除
- [ ] 鍵が取得できない場合は投稿/送信を中止しエラー提示（TASK-154 と連携）
- [ ] 鍵取得ロジックを View から外し、ViewModel/サービス層に集約
- [ ] 同様パターンが他にないか全体を点検

**影響ファイル**: `DriftSonarApp/DriftSonarApp/Views/EncounterView.swift`、`TimelineView.swift`、`ViewModels/`
**依存**: TASK-055
...
- [ ] 共通のエラー表示（アラート/トースト）を定義
- [ ] 暗号失敗・鍵不在・BLE 不可・投稿失敗を分類し適切なメッセージを出す
- [ ] リカバリ可能なものは再試行導線を提供
- [ ] 機微情報をエラー文に出さない

**影響ファイル**: `DriftSonarApp/DriftSonarApp/Views/`、`ViewModels/`
...
- [ ] 起動時に「プロファイル有 ⇔ 対応する鍵が Keychain に有」を検証
- [ ] 不整合時の挙動を定義（再生成案内・データ消去案内など）
- [ ] 公開鍵とプロファイルの対応が壊れていないか確認
- [ ] 検証ロジックを `AppServices` 初期化時に実行

**影響ファイル**: `DriftSonarApp/DriftSonarApp/DriftSonarApp/AppServices.swift`、`ProfileDomain/`
**依存**: TASK-153
